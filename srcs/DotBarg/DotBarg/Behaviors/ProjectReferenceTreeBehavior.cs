 ...
 * 
 * 謎。ProjectReferenceTreeBehavior がうまく個別に対応できている（各ドキュメントペインの CanvasEx だけに対応している）。
 * InheritanceSourceTreeBehavior, InheritanceDestinationTreeBehavior の時の問題との違いはなんだろうか。
 * 分からない。けどうまく動いているみたいからヨシ！
 * 
 */
...
{
    public class ProjectReferenceTreeBehavior : Behavior<CanvasEx>
    {
        ...
        public static readonly DependencyProperty ProjectTreeItemsProperty =
            DependencyProperty.Register(
                nameof(ProjectTreeItems),
                typeof(ObservableCollection<DefinitionItemModel>),
                typeof(ProjectReferenceTreeBehavior),
                new PropertyMetadata(null, OnProjectTreeItemsPropertyChanged));

        public ObservableCollection<DefinitionItemModel> ProjectTreeItems
        {
            get { return GetValue(ProjectTreeItemsProperty) as ObservableCollection<DefinitionItemModel>; }
            set { SetValue(ProjectTreeItemsProperty, value); }
        }

        private static void OnProjectTreeItemsPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
